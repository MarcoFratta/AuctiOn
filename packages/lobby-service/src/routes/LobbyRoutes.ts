...
export const createLobbyRouter = (controller: LobbyController): express.Router => {
  ...
  // Routes
  router.post('/create', validateRequestBody(lobbyConfigSchema), controller.createLobby, errorsMiddlewares);

  router.post('/join/:id', validateRequestParams(lobbyId), controller.joinLobby, errorsMiddlewares);
  ...
  router.post('/leave/:id', validateRequestParams(lobbyId), controller.leaveLobby, errorsMiddlewares);

  router.post('/kick/:id', validateRequestParams(lobbyId), controller.kickPlayer, errorsMiddlewares);

  router.post('/start/:id', validateRequestParams(lobbyId), controller.startMatch, errorsMiddlewares);
